...
export const Rings = ({ readings, realtime }) => {
  for (let i in rings) {
    rings[i].totalProgress = readings * limit[i];
  }
  return (
    <Canvas style={{ flex: 1 }}>
      <Fill color="black" />
      {rings.map((ring, index) => {
        return <Ring key={index} ring={ring} center={center} strokeWidth={strokeWidth} />;
      })}
    </Canvas>
  );
};
